...
export default function TrackerPage() {
  ...
  return (
    <AppShell title="Application Tracker" subtitle="Move scholarships from saved to applying to submitted and stay ahead of each deadline.">
      {!token ? <div className="rounded-lg border border-gray-200 bg-white p-8 text-sm text-gray-600 shadow-sm">Log in to manage your application tracker.</div> : null}
      {token ? (
        <div className="space-y-4">
            ...
            <div key={application.id} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">{application.scholarship.organization}</p>
                  <p className="mt-3 text-sm text-gray-600">Deadline {formatDate(application.scholarship.deadline)}</p>
                </div>
          ...
          )) : <div className="rounded-lg border border-gray-200 bg-white p-8 text-sm text-gray-500 shadow-sm">No applications yet. Save or apply to scholarships from the browse page.</div>}
        </div>
      ) : null}
    </AppShell>
  );
