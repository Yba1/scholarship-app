...
export function DeadlineWidget({ scholarships }: { scholarships: Scholarship[] }) {
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm md:p-6">
      <h3 className="mb-4 text-sm font-semibold text-gray-900 md:text-base">Upcoming Deadlines</h3>
      <div className="space-y-4">
        {scholarships.length ? (
          scholarships.map((scholarship, index) => (
            <div key={scholarship.id} className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-gray-900">{scholarship.title}</p>
          ...
          <p className="text-sm text-gray-500">No deadlines within the next week yet.</p>
        )}
      </div>
    </div>
  );
